    ...
    for item in &ast.items {
        match item {
            syn::ForeignItem::Fn(f) => {
                ...
                let gen_ffi = quote! {
                    #[no_mangle]
                    fn #origin_ident(params_pointer: *mut u32, params_count: i32) -> i32;
                };
                ...
                let wrap_ffi = quote! {
                    #[no_mangle]
                    unsafe fn #ori_run_ident(#(#arg_names : String),*) -> String {
                        let mut params_pointer : u32 = 1;
                        let rvec_pointer: *const Vec<u8> = unsafe { #origin_ident(&mut params_pointer, #params_len as i32) } as *const Vec<u8>;
                        let rvec: Vec<u8> = unsafe { (*rvec_pointer).clone() };
                        let flag = rvec[0];
